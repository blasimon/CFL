...
# Declaring something to store study data if needed
studydata = None
is_verified = False

# function for reading the study config file
...
@app.route('/api/get_verification_state', methods=['GET'])
...
# endpoint that is used to create a link token
@app.route('/api/create_link_token', methods=['POST'])
def create_link_token():
    try:
        request = LinkTokenCreateRequest(
            products=products,
            client_name="Consumer Finance Link",
            country_codes=list(map(lambda x: CountryCode(x), PLAID_COUNTRY_CODES)),
            language='en',
            user=LinkTokenCreateRequestUser(
                client_user_id=str(time.time())
...
# API endpoint created to verify eligibility based on the last transaction
@app.route('/api/verifylasttrans', methods=['GET'])
...
@app.route('/api/balancemin', methods=['GET'])
def get_balancemin():
    json_response = [False]
    try:
